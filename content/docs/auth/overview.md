...
The `neon-auth` agent skill provides comprehensive guidance for the full Neon Auth setup workflow, including provisioning, configuring OAuth providers and trusted origins, setting up the email provider, and integrating the Neon JS SDK. To access this skill, run `npx neonctl@latest init` or `npx skills add neondatabase/agent-skills -s neon-auth` in your project. See [Agent Skills](/docs/ai/agent-skills) for more details.
